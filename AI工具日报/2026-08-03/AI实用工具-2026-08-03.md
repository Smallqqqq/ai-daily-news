...
| Zep / Graphiti | 时序知识图谱 | Docker / 云 API | 事实带有效期窗口，可查询"一月份的项目负责人是谁" | 开源 + 托管 | ⭐ 24K |
  ...
  PMB 跟 Mem0、Letta、Zep 走的不是一条路。后者都是通用 Agent 记忆平台，适合构建需要长期用户画像的聊天应用或客服 Agent。PMB 的定位更窄也更锋利——它只服务编程 Agent（Claude Code、Codex、Cursor），靠生命周期钩子而非工具调用来注入记忆，读路径完全不走 LLM。如果你需要跨应用共享用户画像，Mem0 更合适；如果你需要一个本地跑、零配置、对编程 Agent 开箱即用的记忆层，PMB 是目前最务实的选项。
...
| Cursor | VS Code Fork | 内置 @codebase + 自动检索 | 仅内置模型菜单 | $20-40/月 | 未公开 |
  ...
  Codistry 跟 Cline、Continue、Cursor 不是直接竞争关系——它解决的是另一个问题。Cline 擅长自主执行任务、Continue 擅长补全和代码问答、Cursor 做的是端到端的 AI 编辑器体验。Codistry 的独特性在于**代码库结构理解**这一层：Context Engine 在 Agent 还没开始干活之前，就已经帮它建立了一张项目地图。如果你的日常不是在全新项目里从零写代码，而是在一个已经跑了三年、有 200 个模块的老仓库里做功能开发和维护，这个差异会很直观。但它的 Agent 交互界面（特别是长任务中的方向调整）还没追平 Cline 和 Cursor 的成熟度，这是目前的主要短板。
